...
on_failure() {
    ...
    if [[ -n "$IMESSAGE_TARGET" ]]; then
        ...
        local msg
        if [[ -n "$last_err" ]]; then
            msg="Morning brief failed: $last_err"
        else
            msg="Morning brief failed (exit $exit_code). Check ~/.morning_brief.log"
        fi
        local escaped="${msg//\\/\\\\}"
        escaped="${escaped//\"/\\\"}"
        "$OSASCRIPT_BIN" -e "
...
# Read fresh Google access token from Keychain (shared/refresh_tokens.py just updated it)
...
run_brief() {
    "$VENV_PY" "$SCRIPT_DIR/morning_brief.py" "$@"
}
...
GOOGLE_TOKEN="$("$SECURITY_BIN" find-generic-password -a "$KEYCHAIN_USER" -s "morning-brief-google-token" -w 2>/dev/null)" || {
    log_err "No google token after retry refresh."
    exit 1
}
export GOOGLE_TOKEN
